...
class Replay_Memory():
    ...
    def __init__(self, memory_size=50000, burn_in=10000, combined=False, prioritized=False, hindsight=False, default_goal=None, priority_alpha=None, *args, **kwargs):
    ...
    def sample_batch(self, batch_size, beta=None):
        ...
        if self.combined:
            ...
            latest_exp = self.experiences[int(latest_exp_index)]
    ...
    def append(self, transition):
        ...
        if len(self.experiences) < self.max_mem_size: # when the buffer hasn't reached capacity yet
            ...
            self.experiences[int(self.next_index)] = transition 
        ...
        if self.prioritized:
            self.sum_tree[int(self.next_index)] = self.curr_max_priority ** self.alpha 
            self.min_tree[int(self.next_index)] = self.curr_max_priority ** self.alpha 
